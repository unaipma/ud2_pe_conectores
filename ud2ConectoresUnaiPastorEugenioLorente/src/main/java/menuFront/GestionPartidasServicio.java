...
public class GestionPartidasServicio {
    ...
    private void sincronizarProgresoJugador(Scanner scanner) {
        try {
            if (menuConexion(scanner)) {
                List<PlayerProgress> playerProgresses = sqliteManager.getPlayerProgressById(playerProgress.getNickName());
                for (PlayerProgress part : playerProgresses) {
                    partida.setIdjugador(part.getPlayerId());
                    partida.setExp(part.getExperience());
                    partida.setMonedas(part.getCoins());
                    partida.setNivel(part.getLifeLevel());
                    partida.setIdjuego(part.getPartidaId());

                    partidaDAO.updatePartida(partida);
                    System.out.println("Progreso sincronizado con el servidor.");
                    
                    implementarCambiosVideoJuego();
                }
            }
        } catch (Exception e) {
    ...
    private void implementarCambiosJugador() {
        try {
            ...
            jugador.setSession_count(totalSesiones);
    ...
    private void implementarCambiosVideoJuego() {
        try {
            Videojuego videojuego
                    = videojuegoDAO.getVideojuego(partida.getIdjuego());
            
            videojuego.setTotal_sessions(playerProgress.getSessionCount());
    ...
    private void mostrarEstadisticasJugador(Scanner scanner) {
        try {
            ...
            if (playerProgresses.isEmpty()) {
                ...
                for (PlayerProgress playerProgres : playerProgresses) {
                    System.out.println(playerProgres.toString());
                }
            }
        } catch (SQLException e) {
    ...
    private boolean menuConexion(Scanner scanner) throws SQLException {
        ...
        this.videojuegoDAO = DAOFactory.getVideojuegoDAO(tipoBD);
